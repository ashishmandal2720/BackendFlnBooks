...
const getProfile = async (req, res) => {
  /* #swagger.tags = ['School profile'] */
  /* #swagger.security = [{"Bearer": []}] */
  const user_id = req.user.user_id;
  const users = await pool.query('SELECT user_id, name, email, role_id, status FROM mst_users where role_id = $1 and user_id=$2' , ['7',user_id]);
...
const updateProfile = async (req, res) => {
    ...
    try {
      ...
      const existingUser = await pool.query("SELECT * FROM public.mst_users WHERE user_id = $1 and role_id=$2", [user_id, '7']);
...
const updatePassword = async (req, res) => {
    ...
    try {
      ...
      // Get existing user to preserve old values if new ones aren't provided
      const existingUser = await pool.query("SELECT * FROM public.mst_users WHERE user_id = $1 and role_id=$2", [id, '7']);
      if (existingUser.rows.length === 0) {
        return responseHandler(res, 404, 'User not found');
      }
  ...
  const getStudentCountByUdiseCode = async(udiseCode) => {
    try {
      const result = await pool.query(
        'SELECT * FROM public.central_student_counts WHERE udise_sch_code = $1',
        [udiseCode]
      );
      ...
      console.error('Error in getStudentCountByUdiseCode:', error);
      throw error;
    }
  }
