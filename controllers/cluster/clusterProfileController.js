...
const getProfile = async (req, res) => {
  /* #swagger.tags = ['Cluster Profile'] */
  /* #swagger.security = [{"Bearer": []}] */
  const user_id = req.user.user_id;
  const users = await pool.query('SELECT user_id, name, email, role_id, status FROM mst_users where role_id = $1 and user_id=$2' , ['6',user_id]);
...
const updateProfile = async (req, res) => {
    ...
    try {
      ...
      const existingUser = await pool.query("SELECT * FROM public.mst_users WHERE user_id = $1 and role_id='6'", [id]);
